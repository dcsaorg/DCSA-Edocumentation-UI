 ...
 */
import { BookingRequestCreatedDateTime } from './bookingRequestCreatedDateTime';
import { BookingRequestUpdatedDateTime } from './bookingRequestUpdatedDateTime';

/**
 * The `carrierBookingRequestReference` along with the `documentStatus` is returned 
 */
export interface BookingRefCancelledStatus { 
    /**
     * The Cancelled status of the booking: - CANC (Cancelled)  More details can be found on <a href=\"https://github.com/dcsaorg/DCSA-Information-Model/blob/master/datamodel/referencedata.d/shipmenteventtypecodes.csv\">GitHub</a>. Be aware that the list linked to is the `ShipmentEventTypeCodes` which is equivalent to `documentStatus`, the list is a subset of the possible values. 
     */
    documentStatus: BookingRefCancelledStatus.DocumentStatusEnum;
    bookingRequestCreatedDateTime: BookingRequestCreatedDateTime;
    bookingRequestUpdatedDateTime: BookingRequestUpdatedDateTime;
}
export namespace BookingRefCancelledStatus {
    export type DocumentStatusEnum = 'CANC';
    export const DocumentStatusEnum = {
        CANC: 'CANC' as DocumentStatusEnum
    };
}